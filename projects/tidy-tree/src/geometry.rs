...
mod test {
    ...
    #[test]
    fn intersect() {
        let a = Line { from: Point { x: 0., y: 0. }, to: Point { x: 1., y: 0. } };
        let b = Line { from: Point { x: 1., y: 1. }, to: Point { x: 1., y: -1. } };
        assert!(a.intersect(&b));

        let a = Line { from: Point { x: 0., y: 0. }, to: Point { x: 1., y: 0. } };
        let b = Line { from: Point { x: 2., y: 1. }, to: Point { x: 1., y: -1. } };
        assert!(!a.intersect(&b));

        let a = Line { from: Point { x: 0., y: 0. }, to: Point { x: 1., y: 1. } };
        let b = Line { from: Point { x: 0., y: 1. }, to: Point { x: 1., y: 0. } };
        assert!(a.intersect(&b));

        let a = Line { from: Point { x: 0., y: 0. }, to: Point { x: 1., y: 1. } };
        let b = Line { from: Point { x: 1., y: 1. }, to: Point { x: 2., y: 2. } };
        assert!(a.intersect(&b));

        let a = Line { from: Point { x: 0., y: 0. }, to: Point { x: 1., y: 1. } };
        let b = Line { from: Point { x: 2., y: 2. }, to: Point { x: 3., y: 3. } };
        assert!(!a.intersect(&b));
    }
}
